...
DEPLOYMENT NOTE:
----------------
On Streamlit Cloud, Apriori is too memory-intensive to run live.
This app loads precomputed results from data/raw/precomputed_*.csv
These files were generated locally and committed to the repository.
To regenerate: python scripts/precompute.py
"""
...
# ===========================================================
# DATA LOADING — from precomputed CSV files
# ===========================================================

@st.cache_data
def load_data():
    """
    Load precomputed pipeline outputs from CSV files.
    Apriori runs locally and results are committed to the repo.
    This avoids the memory limit on Streamlit Cloud free tier.
    """
    data_dir = Path(__file__).parent.parent / "data" / "raw"

    # Load DataFrames
    baskets_df    = pd.read_csv(data_dir / "precomputed_baskets.csv")
    freq_df       = pd.read_csv(data_dir / "precomputed_freq.csv")
    top_customers = pd.read_csv(data_dir / "precomputed_top_customers.csv")
    rules_df      = pd.read_csv(data_dir / "precomputed_rules.csv")

    # Reconstruct frozensets from pipe-separated strings
    ...
    # Load JSON stats
    with open(data_dir / "precomputed_stats.json") as f:
        stats = json.load(f)

    with open(data_dir / "precomputed_summary.json") as f:
        summary = json.load(f)

    # Load categories count from freq_df
    categories_count = len(freq_df)
